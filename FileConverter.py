...
def extract_nodule_information(n, nodule_class):
	...
	SliceLocation = findall(n, filter_=lambda node: node.name.ID not in ("imageZposition"))
					...
					for uid in x.imageSOP_UID:
						...
						if f.SOPInstanceUID == uid:
							...
							if f.pixel_array.shape == (512, 512):
								order.append([i, f.SliceLocation])
								i += 1
...
if __name__ == "__main__":
	...
	path = "raw_data/"
	output_path = "dataset_128/"
	output_size = 128
	pre_processing_step = "morphed"
